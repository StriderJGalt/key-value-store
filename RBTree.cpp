...
int kvstore::getColor(Node *&node) {
    if (node == nullptr)
        return BLACK;
...
void kvstore::setColor(Node *&node, int color) {
    if (node == nullptr)
        return;
...
Node* kvstore::insertBST(Node *&root, Node *&ptr) {
    if (root == nullptr)
        return ptr;
...
void kvstore::insertValue(int n) {
    Node *node = new Node(n);
    root = insertBST(root, node);
    fixInsertRBTree(node);
}

void kvstore::rotateLeft(Node *&ptr) {
...
void kvstore::rotateRight(Node *&ptr) {
...
void kvstore::fixInsertRBTree(Node *&ptr) {
    Node *parent = nullptr;
...
void kvstore::fixDeleteRBTree(Node *&node) {
    if (node == nullptr)
        return;
...
Node* kvstore::deleteBST(Node *&root, int data) {
    if (root == nullptr)
        return root;
...
void kvstore::deleteValue(int data) {
    Node *node = deleteBST(root, data);
    fixDeleteRBTree(node);
}

void kvstore::inorderBST(Node *&ptr) {
    if (ptr == nullptr)
        return;
...
void kvstore::inorder() {
    inorderBST(root);
}

void kvstore::preorderBST(Node *&ptr) {
    if (ptr == nullptr)
        return;
...
void kvstore::preorder() {
    preorderBST(root);
    cout << "-------" << endl;
}

Node *kvstore::minValueNode(Node *&node) {
...
Node* kvstore::maxValueNode(Node *&node) {
    Node *ptr = node;
...
int kvstore::getBlackHeight(Node *node) {
    int blackheight = 0;
    while (node != nullptr) {
...
void kvstore::merge(RBTree rbTree2) {
    int temp;
    Node *c, *temp_ptr;
    Node *root1 = root;
